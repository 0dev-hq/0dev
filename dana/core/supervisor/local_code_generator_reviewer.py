...
class LocalCodeGeneratorReviewer:
    ...
    def _generate_review_prompt(
        ...
        SYSTEM_PROMPT = """You are a highly skilled code reviewer responsible for verifying the correctness of a Python code and
        its alignment with the specified requirements. The code is generated by an AI model and will be automatically executed
        to perform a specific task, so if the code doesn't follow specific guidelines, it may lead to unexpected behavior. Your
        main goal is to ensure the code follows these guidelines and also actually performs the task as expected.
